...
def generate_design_output(design_problem, ontology_element, role_description, ontology_element_definition, ontology_element_example):
    prompt = (
        ...
        #    f"--- END OF TASK ---\n"
        f"Design Goal: '{design_problem}'\n"
            f"Your task is to propose as many distinct and creative {ontology_element} for the given Design Goal.\n"
            f"- '{ontology_element_definition}'\n"
            f"- 'For example, for a turbocharger {ontology_element} include elements such as: {ontology_element_example}'\n"
            f"- Generate diverse {ontology_element} that explore different possibilities.\n"
            f"- Ensure that each {ontology_element} aligns correctly with its category.\n"
            f"- Do not limit solutions to feasibility—some may be abstract or exploratory in nature.\n"
            f"- Provide all the solutions as a single list, separated by commas, without additional explanation or numbering.\n"
            f"Correctness and diversity are the most important criteria, followed by quantity and relevance.\n"
            f"--- END OF TASK ---\n"
    )
    ...
    try:
        ...
        if not hasattr(response, "choices") or not response.choices:
            st.warning("The API response did not contain valid choices.")
            return []
        ...
        if not content:
            st.warning("The response was empty.")
            return []
        if "," not in content:
            st.warning(f"Unexpected response format: {content}")
            return []
